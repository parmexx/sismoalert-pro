...
def obtener_sismos(dias: int, magnitud_min: float, region_params: dict) -> pd.DataFrame:
    ...
    try:
        ...
        resp = requests.get(USGS_API, params=params_usgs, timeout=15)
        resp.raise_for_status()
        for f in resp.json().get("features", []):
            eid = f"usgs_{f.get('id','')}"
        ...
        st.warning(f"⚠️ USGS no disponible: {e}")

    # ── EMSC (cobertura mundial) ───────────────────────────
    try:
        params_emsc = {
            "format":    "json",
            "starttime": fecha_inicio.strftime("%Y-%m-%d"),
        ...
        # Si hay region_params con lat/lon los aplicamos
        if "minlatitude" in region_params:
            params_emsc["minlat"] = region_params["minlatitude"]
            params_emsc["maxlat"] = region_params["maxlatitude"]
            params_emsc["minlon"] = region_params["minlongitude"]
            params_emsc["maxlon"] = region_params["maxlongitude"]

        resp = requests.get(EMSC_API, params=params_emsc, timeout=15)
        resp.raise_for_status()
        for f in resp.json().get("features", []):
            p   = f.get("properties", {})
            geo = f.get("geometry", {}).get("coordinates") or [None, None, None]
            eid = f"emsc_{f.get('id') or p.get('unid','')}"
            if eid in ids_vistos or not geo[0]:
                continue
            ids_vistos.add(eid)
            ts = p.get("time")
            if isinstance(ts, str):
                ...
                continue
            registros.append({
                "lugar":       p.get("flynn_region") or p.get("place") or "Desconocido",
                "magnitud":    float(p.get("mag") or 0),
                "profundidad": float(geo[2]) if geo[2] else 0,
                "lon":         float(geo[0]),
                "lat":         float(geo[1]),
                "tiempo":      tiempo,
                "tipo":        "earthquake",
    ...
    df = df[df["magnitud"] >= magnitud_min]
    df = df.sort_values("tiempo", ascending=False).reset_index(drop=True)
    return df
